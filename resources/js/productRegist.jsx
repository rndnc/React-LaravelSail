...
function ProductRegist () {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await fetch(`/api/products/getCompany`, {
                    method: 'POST',
                    headers: {
    ...
    const handleSubmit  = async (e)=> {
        ...
        try{
            ...
            const response = await fetch(`/api/products/registProduct`, {
                method: 'POST',
                body: formData,
            });
    ...
    return(
        ...
        <div class="col-md-4 ms-4 mt-4">
            ...
            <input class="form-control md-6" type="text" name="product_name" onChange={(e) => setProduct_name(e.target.value)} />
        ...
        <div class="col-md-4 ms-4 mt-2">
            <label htmlFor="company_id"  class="form-label">メーカー名</label>
                <select class="form-select md-6" id="company_id" name="company_id" onChange={(e) => setCompany_id(e.target.value)}>
        ...
        <div class="col-md-4 ms-4 mt-2">
            ...
            <input class="form-control md-6" type="text" name="price" onChange={(e) => setPrice(e.target.value)}/>
            {errors.price && <p className="text-danger">{errors.price[0]}</p>}
        </div>
        <div class="col-md-4 ms-4 mt-2">
            <label htmlFor="stock" class="form-label">在庫</label>
            <input class="form-control md-6" type="text" name="stock" onChange={(e) => setStock(e.target.value)}/>
            {errors.stock && <p className="text-danger">{errors.stock[0]}</p>}
        </div>
        <div class="col-md-4 ms-4 mt-2">
            <label htmlFor="comment" class="form-label">コメント</label>
            <textarea class="form-control md-6" type="text" name="comment" onChange={(e) => setComment(e.target.value)}/>
            {errors.comment && <p className="text-danger">{errors.comment[0]}</p>}
        </div>
        <div class="col-md-4 ms-4 mt-2">
            <label htmlFor="img_path" >画像</label>
...
const productRegist = document.getElementById("product_regist");
if (productRegist) {
    const root = createRoot(productRegist);
    root.render(<ProductRegist />);
}
